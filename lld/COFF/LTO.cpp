...
lto::Config BitcodeCompiler::createConfig() {
  ...
  if (ctx.config.saveTemps)
    checkError(ctx, c.addSaveTemps(std::string(ctx.config.outputFile) + ".",
...
static void undefine(COFFLinkerContext &ctx, Symbol *s) { replaceSymbol<Undefined>(s, s->getName(ctx)); }
...
std::vector<InputFile *> BitcodeCompiler::compile() {
  ...
  if (!ctx.config.ltoCache.empty())
    cache = check(ctx, localCache("ThinLTO", "Thin", ctx.config.ltoCache,
  ...
  for (unsigned i = 0; i != maxTasks; ++i) {
    ...
    if (!emitASM)
      ret.push_back(ctx.make<ObjFile>(ctx, MemoryBufferRef(objBuf, ltoObjName)));
  }
