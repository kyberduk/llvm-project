...
static std::string maybeDemangleSymbol(Ctx &ctx, StringRef symName) {
  return ctx.config->demangle ? demangle(symName.str()) : symName.str();
}

std::string lld::toString(elf::Ctx &ctx, const elf::Symbol &sym) {
...
static uint64_t getSymVA(Ctx &ctx, const Symbol &sym, int64_t addend) {
  ...
  case Symbol::DefinedKind: {
    ...
    uint64_t va = isec->getVA(ctx, offset);
    if (d.isSection())
      va -= addend;
    ...
    if (ctx.config->emachine == EM_MIPS && isMicroMips(ctx) &&
        ((sym.stOther & STO_MIPS_MICROMIPS) || sym.hasFlag(NEEDS_COPY)))
      va |= 1;

    if (d.isTls() && !ctx.config->relocatable) {
      ...
      if (!ctx.out.tlsPhdr || !ctx.out.tlsPhdr->firstSec)
        ctx.fatal(toString(ctx, d.file) +
                  " has an STT_TLS symbol but doesn't have an SHF_TLS section");
      return va - ctx.out.tlsPhdr->firstSec->addr;
    }
    return va;
  }
...
uint32_t Symbol::getGotIdx(Ctx &ctx) const { return ctx.symAux[auxIdx].gotIdx; }
uint32_t Symbol::getPltIdx(Ctx &ctx) const { return ctx.symAux[auxIdx].pltIdx; }
uint32_t Symbol::getTlsDescIdx(Ctx &ctx) const {
  return ctx.symAux[auxIdx].tlsDescIdx;
}
uint32_t Symbol::getTlsGdIdx(Ctx &ctx) const {
  return ctx.symAux[auxIdx].tlsGdIdx;
}
...
uint64_t Symbol::getGotVA(Ctx &ctx) const {
  if (gotInIgot)
    return ctx.in.igotPlt->getVA(ctx) + getGotPltOffset(ctx);
  return ctx.in.got->getVA(ctx) + getGotOffset(ctx);
}
...
uint64_t Symbol::getGotPltVA(Ctx &ctx) const {
  if (isInIplt)
    return ctx.in.igotPlt->getVA(ctx) + getGotPltOffset(ctx);
  return ctx.in.gotPlt->getVA(ctx) + getGotPltOffset(ctx);
}
...
uint64_t Symbol::getPltVA(Ctx &ctx) const {
  uint64_t outVA = isInIplt ? ctx.in.iplt->getVA(ctx) +
                                  getPltIdx(ctx) * ctx.target->ipltEntrySize
                            : ctx.in.plt->getVA(ctx) + ctx.in.plt->headerSize +
...
void Symbol::parseSymbolVersion(Ctx &ctx) {
  ...
  // symbol table.
  if (ctx.config->shared && versionId != VER_NDX_LOCAL)
    ctx.error(toString(ctx, file) + ": symbol " + s +
              " has undefined version " + verstr);
}
...
uint8_t Symbol::computeBinding(Ctx &ctx) const {
  ...
  if ((v != STV_DEFAULT && v != STV_PROTECTED) || versionId == VER_NDX_LOCAL)
    return STB_LOCAL;
  if (binding == STB_GNU_UNIQUE && !ctx.config->gnuUnique)
    return STB_GLOBAL;
  return binding;
}
...
bool Symbol::includeInDynsym(Ctx &ctx) const {
  ...
  if (!isDefined() && !isCommon())
    ...
    return !(isUndefWeak() && ctx.config->noDynamicLinker);

  return exportDynamic || inDynamicList;
}

// Print out a log message for --trace-symbol.
void elf::printTraceSymbol(Ctx &ctx, const Symbol &sym, StringRef name) {
  std::string s;
  if (sym.isUndefined())
    s = ": reference to ";
  else if (sym.isLazy())
  ...
  ctx.message(toString(ctx, sym.file) + s + name);
}

static void recordWhyExtract(Ctx &ctx, const InputFile *reference,
                             const InputFile &extracted, const Symbol &sym) {
  ctx.whyExtractRecords.emplace_back(toString(ctx, reference), &extracted, sym);
}
...
void elf::maybeWarnUnorderableSymbol(Ctx &ctx, const Symbol *sym) {
  ...
  auto report = [&](StringRef s) {
    ctx.warn(toString(ctx, file) + s + sym->getName());
  };
...
bool elf::computeIsPreemptible(Ctx &ctx, const Symbol &sym) {
  ...
  // -Bsymbolic-functions.
  if (ctx.config->symbolic ||
      (ctx.config->bsymbolic == BsymbolicKind::NonWeak &&
       sym.binding != STB_WEAK) ||
      (ctx.config->bsymbolic == BsymbolicKind::Functions && sym.isFunc()) ||
      (ctx.config->bsymbolic == BsymbolicKind::NonWeakFunctions &&
       sym.isFunc() && sym.binding != STB_WEAK))
...
void Symbol::resolve(Ctx &ctx, const Undefined &other) {
  ...
  if (isPlaceholder() || (isShared() && other.visibility() != STV_DEFAULT) ||
    ...
    other.overwrite(ctx, *this);
    return;
  }

  if (traced)
    printTraceSymbol(ctx, other, getName());
  ...
  if (isLazy()) {
    ...
    bool backref = ctx.config->warnBackrefs && other.file &&
...
bool Symbol::shouldReplace(Ctx &ctx, const Defined &other) const {
  if (LLVM_UNLIKELY(isCommon())) {
    if (ctx.config->warnCommon)
      ctx.warn("common " + getName() + " is overridden");
    return !other.isWeak();
  }
  if (!isDefined())
    return true;
...
void Symbol::allocateAux(Ctx &ctx) {
  assert(auxIdx == 0);
...
void elf::reportDuplicate(Ctx &ctx, const Symbol &sym, const InputFile *newFile,
  ...
  if (ctx.config->allowMultipleDefinition)
    return;
  ...
  if (!d->section && !errSec && errOffset && d->value == errOffset)
    return;
  if (!d->section || !errSec) {
    ctx.error("duplicate symbol: " + toString(ctx, sym) + "\n>>> defined in " +
              toString(ctx, sym.file) + "\n>>> defined in " +
              toString(ctx, newFile));
    return;
  }
  ...
  std::string src1 = sec1->getSrcMsg(ctx, sym, d->value);
  std::string obj1 = sec1->getObjMsg(ctx, d->value);
  std::string src2 = errSec->getSrcMsg(ctx, sym, errOffset);
  std::string obj2 = errSec->getObjMsg(ctx, errOffset);

  std::string msg =
      "duplicate symbol: " + toString(ctx, sym) + "\n>>> defined at ";
  if (!src1.empty())
    msg += src1 + "\n>>>            ";
  msg += obj1 + "\n>>> defined at ";
  if (!src2.empty())
...
void Symbol::resolve(Ctx &ctx, const CommonSymbol &other) {
  ...
  if (CommonSymbol *oldSym = dyn_cast<CommonSymbol>(this)) {
    if (ctx.config->warnCommon)
      ctx.warn("multiple common of " + getName());
...
void Symbol::resolve(Ctx &ctx, const LazySymbol &other) {
  ...
  if (LLVM_UNLIKELY(isCommon()) && ctx.config->fortranCommon &&
      other.file->shouldExtractForCommon(ctx, getName())) {
...
Defined::Defined(Ctx &ctx, InputFile *file, StringRef name, uint8_t binding,
                 uint8_t stOther, uint8_t type, uint64_t value, uint64_t size,
                 SectionBase *section)
    : Symbol(DefinedKind, file, name, binding, stOther, type), value(value),
      size(size), section(section) {
  exportDynamic = ctx.config->exportDynamic;
}

void Defined::overwrite(Ctx &ctx, Symbol &sym) const {
  if (isa_and_nonnull<SharedFile>(sym.file))
    sym.versionId = VER_NDX_GLOBAL;
  Symbol::overwrite(ctx, sym, DefinedKind);
...
CommonSymbol::CommonSymbol(Ctx &ctx, InputFile *file, StringRef name,
