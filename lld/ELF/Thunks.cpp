...
public:
  PPC32LongThunk(Ctx &ctx, Symbol &dest, int64_t addend)
      : Thunk(ctx, dest, addend) {}
  uint32_t size() override { return ctx.config->isPic ? 32 : 16; }
  void writeTo(uint8_t *buf) override;
  void addSymbols(ThunkSection &isec) override;
};
  ...
  PPC64PILongBranchThunk(Ctx &ctx, Symbol &dest, int64_t addend)
    ...
    if (std::optional<uint32_t> index =
      ...
      ctx.mainPart->relaDyn->addRelativeReloc(
          ctx, ctx.target->relativeRel, *ctx.in.ppc64LongBranchTarget,
...
bool ARMThunk::isCompatibleWith(const InputSection &isec,
  ...
  if (!ctx.config->armHasBlx && rel.type == R_ARM_THM_CALL)
    return false;
...
bool ThumbThunk::getMayUseShortThunk() {
  if (!mayUseShortThunk || !ctx.config->armJ1J2BranchEncoding)
    return false;
  uint64_t s = getARMThunkDestVA(ctx, destination);
  if ((s & 1) == 0) {
    mayUseShortThunk = false;
...
bool ThumbThunk::isCompatibleWith(const InputSection &isec,
  ...
  if (!ctx.config->armHasBlx && rel.type == R_ARM_CALL)
    return false;
...
void elf::writePPC32PltCallStub(Ctx &ctx, uint8_t *buf, uint64_t gotPltVA,
  ...
  if (addend >= 0x8000) {
    ...
    offset = gotPltVA -
             (ctx.in.ppc32Got2->getParent()->getVA(ctx) +
    ...
    offset = gotPltVA - ctx.in.got->getVA(ctx);
  }
...
void PPC32PltCallStub::addSymbols(ThunkSection &isec) {
  ...
  addSymbol(ctx.saver.save(os.str()), STT_FUNC, 0, isec);
}

bool PPC32PltCallStub::isCompatibleWith(const InputSection &isec,
                                        const Relocation &rel) const {
  return !ctx.config->isPic || (isec.file == file && rel.addend == addend);
}
...
void PPC64R2SaveStub::writeTo(uint8_t *buf) {
  ...
  if (getMayUseShortThunk()) {
    ...
    const int64_t offsetFromTOC =
        ctx.in.ppc64LongBranchTarget->getEntryVA(ctx, &destination, addend) -
        getPPC64TocBase(ctx);
...
void PPC64R12SetupStub::writeTo(uint8_t *buf) {
  ...
  if (ctx.config->power10Stubs) {
    ...
    writePrefixedInstruction(
        ctx, buf, (gotPlt ? PLD_R12_NO_DISP : PADDI_R12_NO_DISP) | imm);
    nextInstOffset = 8;
  } else {
...
static Thunk *addThunkAArch64(Ctx &ctx, RelType type, Symbol &s, int64_t a) {
  if (type != R_AARCH64_CALL26 && type != R_AARCH64_JUMP26 &&
    ...
    ctx.fatal("unrecognized relocation type");
  if (ctx.config->picThunk)
    return ctx.make<AArch64ADRPThunk>(ctx, s, a);
  return ctx.make<AArch64ABSLongThunk>(ctx, s, a);
}
...
static Thunk *addThunkArmv4(Ctx &ctx, RelType reloc, Symbol &s, int64_t a) {
  ...
  case R_ARM_CALL:
    if (ctx.config->picThunk) {
      if (thumb_target)
        return ctx.make<ARMV4PILongBXThunk>(ctx, s, a);
      return ctx.make<ARMV4PILongThunk>(ctx, s, a);
    }
    if (thumb_target)
      return ctx.make<ARMV4ABSLongBXThunk>(ctx, s, a);
    return ctx.make<ARMV5LongLdrPcThunk>(ctx, s, a);
  case R_ARM_THM_CALL:
    if (ctx.config->picThunk) {
      if (thumb_target)
        return ctx.make<ThumbV4PILongThunk>(ctx, s, a);
      return ctx.make<ThumbV4PILongBXThunk>(ctx, s, a);
    }
    if (thumb_target)
      return ctx.make<ThumbV4ABSLongThunk>(ctx, s, a);
    return ctx.make<ThumbV4ABSLongBXThunk>(ctx, s, a);
  }
  ctx.fatal("relocation " + toString(ctx, reloc) + " to " + toString(ctx, s) +
...
static Thunk *addThunkV6M(Ctx &ctx, const InputSection &isec, RelType reloc,
  ...
  case R_ARM_THM_CALL:
    if (ctx.config->isPic) {
      if (!isPureCode)
        return ctx.make<ThumbV6MPILongThunk>(ctx, s, a);

      ctx.fatal("relocation " + toString(ctx, reloc) + " to " +
                toString(ctx, s) +
                " not supported for Armv6-M targets for position independant"
                " and execute only code");
    }
    if (isPureCode)
      return ctx.make<ThumbV6MABSXOLongThunk>(ctx, s, a);
    return ctx.make<ThumbV6MABSLongThunk>(ctx, s, a);
  }
  ctx.fatal("relocation " + toString(ctx, reloc) + " to " + toString(ctx, s) +
...
static Thunk *addThunkArm(Ctx &ctx, const InputSection &isec, RelType reloc,
  ...
  case R_ARM_CALL:
    if (ctx.config->picThunk)
      return ctx.make<ARMV7PILongThunk>(ctx, s, a);
    return ctx.make<ARMV7ABSLongThunk>(ctx, s, a);
  case R_ARM_THM_JUMP19:
  case R_ARM_THM_JUMP24:
  case R_ARM_THM_CALL:
    if (ctx.config->picThunk)
      return ctx.make<ThumbV7PILongThunk>(ctx, s, a);
    return ctx.make<ThumbV7ABSLongThunk>(ctx, s, a);
  }
  ctx.fatal("unrecognized relocation type");
}
...
static Thunk *addThunkAVR(Ctx &ctx, RelType type, Symbol &s, int64_t a) {
  ...
  case R_AVR_HI8_LDI_GS:
    return ctx.make<AVRThunk>(ctx, s, a);
  default:
    ctx.fatal("unrecognized relocation type " + toString(ctx, type));
  }
}

static Thunk *addThunkMips(Ctx &ctx, RelType type, Symbol &s) {
  if ((s.stOther & STO_MIPS_MICROMIPS) && isMipsR6(ctx))
    return ctx.make<MicroMipsR6Thunk>(ctx, s);
  if (s.stOther & STO_MIPS_MICROMIPS)
    return ctx.make<MicroMipsThunk>(ctx, s);
  return ctx.make<MipsThunk>(ctx, s);
}
...
static Thunk *addThunkPPC32(Ctx &ctx, const InputSection &isec,
  ...
  if (s.isInPlt(ctx))
    return ctx.make<PPC32PltCallStub>(ctx, isec, rel, s);
  return ctx.make<PPC32LongThunk>(ctx, s, rel.addend);
}
...
static Thunk *addThunkPPC64(Ctx &ctx, RelType type, Symbol &s, int64_t a) {
  ...
  if (s.isInPlt(ctx))
    return type == R_PPC64_REL24_NOTOC
               ? (Thunk *)ctx.make<PPC64R12SetupStub>(ctx, s, /*gotPlt=*/true)
               : (Thunk *)ctx.make<PPC64PltCallStub>(ctx, s);
  ...
  if ((type == R_PPC64_REL14 || type == R_PPC64_REL24) && (s.stOther >> 5) == 1)
    return ctx.make<PPC64R2SaveStub>(ctx, s, a);

  if (type == R_PPC64_REL24_NOTOC)
    return ctx.make<PPC64R12SetupStub>(ctx, s, /*gotPlt=*/false);

  if (ctx.config->picThunk)
    return ctx.make<PPC64PILongBranchThunk>(ctx, s, a);

  return ctx.make<PPC64PDLongBranchThunk>(ctx, s, a);
}
