...
RelExpr MIPS<ELFT>::getRelExpr(RelType type, const Symbol &s,
  ...
  if (ELFT::Is64Bits || ctx.config->mipsN32Abi)
    type &= 0xff;
  ...
  case R_MIPS_JALR:
    ...
    if (!s.isFunc() && s.type != STT_NOTYPE) {
      ctx.warn(getErrorLocation(ctx, loc) +
  ...
  case R_MICROMIPS_LO16:
    ...
    if (&s == ctx.es.mipsGpDisp)
      return R_MIPS_GOT_GP_PC;
    if (&s == ctx.es.mipsLocalGp)
...
void MIPS<ELFT>::writeGotPlt(uint8_t *buf, const Symbol &) const {
  uint64_t va = ctx.in.plt->getVA(ctx);
  if (isMicroMips(ctx))
    va |= 1;
  write32(ctx, buf, va);
}
...
template <endianness E>
static void writeShuffleValue(Ctx &ctx, uint8_t *loc, uint64_t v,
...
template <class ELFT> void MIPS<ELFT>::writePltHeader(uint8_t *buf) const {
  ...
  uint32_t jalrInst = ctx.config->zHazardplt ? 0x0320fc09 : 0x0320f809;
...
void MIPS<ELFT>::writePlt(uint8_t *buf, const Symbol &sym,
  ...
  uint32_t jrInst = isMipsR6(ctx)
                        ? (ctx.config->zHazardplt ? 0x03200409 : 0x03200009)
                        : (ctx.config->zHazardplt ? 0x03200408 : 0x03200008);
...
bool MIPS<ELFT>::needsThunk(RelExpr expr, RelType type, const InputFile *file,
  ...
  if (f->getObj(ctx).getHeader().e_flags & EF_MIPS_PIC)
    return false;
  auto *d = dyn_cast<Defined>(&s);
  // LA25 is required if target file has PIC code
  // or target symbol is a PIC symbol.
  return d && isMipsPIC<ELFT>(ctx, d);
}
...
calculateMipsRelChain(Ctx &ctx, uint8_t *loc, RelType type, uint64_t val) {
  ...
  ctx.error(getErrorLocation(ctx, loc) +
            "unsupported relocations combination " + Twine(type));
...
static uint64_t fixupCrossModeJump(Ctx &ctx, uint8_t *loc, RelType type,
  ...
  case R_MICROMIPS_26_S1: {
    ...
    if (inst == 0x3d || inst == 0x3c) { // JAL32 or JALX32
      val >>= 1;
      writeShuffleValue<e>(ctx, loc, 0x3c << 26, 32, 0);
      return val;
    }
    break;
  }
...
void MIPS<ELFT>::relocate(uint8_t *loc, const Relocation &rel,
  ...
  case R_MICROMIPS_GOT16:
    if (ctx.config->relocatable) {
      writeShuffleValue<e>(ctx, loc, val + 0x8000, 16, 16);
    } else {
      checkInt(ctx, loc, val, 16, rel);
      writeShuffleValue<e>(ctx, loc, val, 16, 0);
    }
    break;
  case R_MIPS_CALL16:
  case R_MIPS_GOT_DISP:
  ...
  case R_MICROMIPS_GPREL7_S2:
    ...
    writeShuffleValue<e>(ctx, loc, val, 7, 2);
    break;
  case R_MIPS_CALL_HI16:
  case R_MIPS_GOT_HI16:
  case R_MIPS_HI16:
  ...
  case R_MICROMIPS_PC26_S1:
    ...
    writeShuffleValue<e>(ctx, loc, val, 26, 1);
    break;
  case R_MICROMIPS_PC7_S1:
  ...
  case R_MICROMIPS_PC16_S1:
    ...
    writeShuffleValue<e>(ctx, loc, val, 16, 1);
    break;
  case R_MICROMIPS_PC18_S3:
    checkInt(ctx, loc, val, 21, rel);
    writeShuffleValue<e>(ctx, loc, val, 18, 3);
    break;
  case R_MICROMIPS_PC19_S2:
    checkInt(ctx, loc, val, 21, rel);
    writeShuffleValue<e>(ctx, loc, val, 19, 2);
    break;
  case R_MICROMIPS_PC21_S1:
    checkInt(ctx, loc, val, 22, rel);
    writeShuffleValue<e>(ctx, loc, val, 21, 1);
    break;
  case R_MICROMIPS_PC23_S2:
    checkInt(ctx, loc, val, 25, rel);
    writeShuffleValue<e>(ctx, loc, val, 23, 2);
    break;
  default:
...
template <class ELFT> bool elf::isMipsPIC(Ctx &ctx, const Defined *sym) {
  if (!sym->isFunc())
    return false;
  ...
  return cast<ObjFile<ELFT>>(file)->getObj(ctx).getHeader().e_flags &
         EF_MIPS_PIC;
}

template <class ELFT> TargetInfo *elf::getMipsTargetInfo(Ctx &ctx) {
  return new MIPS<ELFT>(ctx);
}

template TargetInfo *elf::getMipsTargetInfo<ELF32LE>(Ctx &ctx);
template TargetInfo *elf::getMipsTargetInfo<ELF32BE>(Ctx &ctx);
template TargetInfo *elf::getMipsTargetInfo<ELF64LE>(Ctx &ctx);
template TargetInfo *elf::getMipsTargetInfo<ELF64BE>(Ctx &ctx);

template bool elf::isMipsPIC<ELF32LE>(Ctx &ctx, const Defined *);
template bool elf::isMipsPIC<ELF32BE>(Ctx &ctx, const Defined *);
template bool elf::isMipsPIC<ELF64LE>(Ctx &ctx, const Defined *);
template bool elf::isMipsPIC<ELF64BE>(Ctx &ctx, const Defined *);
