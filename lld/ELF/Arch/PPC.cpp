...
void elf::writePPC32GlinkSection(Ctx &ctx, uint8_t *buf, size_t numEntries) {
  ...
  uint32_t glink = ctx.in.plt->getVA(ctx); // VA of .glink
  if (!ctx.config->isPic) {
  ...
  uint32_t got = ctx.in.got->getVA(ctx);
...
void PPC::writeGotHeader(uint8_t *buf) const {
  ...
  write32(ctx, buf, ctx.mainPart->dynamic->getVA(ctx));
}
...
void PPC::relaxTlsIeToLe(uint8_t *loc, const Relocation &rel,
  ...
  case R_PPC_TLS: {
    ...
    if (insn >> 26 != 31)
      ctx.error("unrecognized instruction for IE to LE R_PPC_TLS");
...
TargetInfo *elf::getPPCTargetInfo(Ctx &ctx) { return new PPC(ctx); }
