...
void SystemZ::writeGotHeader(uint8_t *buf) const {
  ...
  write64be(buf, ctx.mainPart->dynamic->getVA(ctx));
}
...
void SystemZ::writePltHeader(uint8_t *buf) const {
  ...
  uint64_t got = ctx.in.got->getVA(ctx);
...
void SystemZ::writePlt(uint8_t *buf, const Symbol &sym,
  ...
  write32be(buf + 24, (ctx.in.plt->getVA(ctx) - pltEntryAddr - 22) >> 1);
...
RelExpr SystemZ::adjustGotPcExpr(RelType type, int64_t addend,
  ...
  if (!ctx.config->relax || addend != 2 || type != R_390_GOTENT)
    return R_GOT_PC;
    ...
    for (InputSection *sec : getInputSections(*osec, storage)) {
      for (Relocation &rel : sec->relocs()) {
        ...
        uint64_t v = sec->getRelocTargetVA(
            ctx, sec->file, rel.type, rel.addend,
...
TargetInfo *elf::getSystemZTargetInfo(Ctx &ctx) { return new SystemZ(ctx); }
