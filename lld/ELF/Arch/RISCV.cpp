...
constexpr uint64_t dtpOffset = 0x800;
...
static uint32_t getEFlags(Ctx &ctx, InputFile *f) {
  if (ctx.config->is64)
    return cast<ObjFile<ELF64LE>>(f)->getObj(ctx).getHeader().e_flags;
  return cast<ObjFile<ELF32LE>>(f)->getObj(ctx).getHeader().e_flags;
}
  ...
  for (InputFile *f : ctx.objectFiles) {
    ...
    if ((eflags & EF_RISCV_FLOAT_ABI) != (target & EF_RISCV_FLOAT_ABI))
      ctx.error(
          ...
          toString(ctx, ctx.objectFiles[0]));
...
int64_t RISCV::getImplicitAddend(const uint8_t *buf, RelType type) const {
  ...
  case R_RISCV_IRELATIVE:
    return ctx.config->is64 ? read64le(buf) : read32le(buf);
  case R_RISCV_NONE:
  case R_RISCV_JUMP_SLOT:
    // These relocations are defined as not having an implicit addend.
    return 0;
  case R_RISCV_TLSDESC:
    return ctx.config->is64 ? read64le(buf + 8) : read32le(buf + 4);
  }
}
...
void RISCV::writeGotHeader(uint8_t *buf) const {
  if (ctx.config->is64)
    ...
    write32le(buf, ctx.mainPart->dynamic->getVA(ctx));
}

void RISCV::writeGotPlt(uint8_t *buf, const Symbol &s) const {
  if (ctx.config->is64)
    write64le(buf, ctx.in.plt->getVA(ctx));
  else
    write32le(buf, ctx.in.plt->getVA(ctx));
}
...
void RISCV::writePltHeader(uint8_t *buf) const {
  ...
  // jr t3
  uint32_t offset = ctx.in.gotPlt->getVA(ctx) - ctx.in.plt->getVA(ctx);
  uint32_t load = ctx.config->is64 ? LD : LW;
  ...
  write32le(buf + 20, itype(SRLI, X_T1, X_T1, ctx.config->is64 ? 1 : 2));
  write32le(buf + 24, itype(load, X_T0, X_T0, ctx.config->wordsize));
...
void RISCV::writePlt(uint8_t *buf, const Symbol &sym,
  ...
  write32le(buf + 4,
            itype(ctx.config->is64 ? LD : LW, X_T3, X_T3, lo12(offset)));
...
RelExpr RISCV::getRelExpr(const RelType type, const Symbol &s,
  ...
  case R_RISCV_RELAX:
    return ctx.config->relax ? R_RELAX_HINT : R_NONE;
...
void RISCV::relocate(uint8_t *loc, const Relocation &rel, uint64_t val) const {
  const unsigned bits = ctx.config->wordsize * 8;
  ...
  case INTERNAL_R_RISCV_GPREL_S: {
    Defined *gp = ctx.es.riscvGlobalPointer;
      ...
      if (i + 1 < size) {
        ...
        if (rel.type == R_RISCV_SET_ULEB128 &&
          ...
          if (overwriteULEB128(loc, val) >= 0x80)
            errorOrWarn(ctx, sec.getLocation(ctx, rel.offset) +
                                 ": ULEB128 value " + Twine(val) +
                                 " exceeds available space; references '" +
                                 lld::toString(ctx, *rel.sym) + "'");
          ++i;
          continue;
        }
      }
      errorOrWarn(
          ctx, sec.getLocation(ctx, rel.offset) +
...
static void relaxHi20Lo12(Ctx &ctx, const InputSection &sec, size_t i,
                          uint64_t loc, Relocation &r, uint32_t &remove) {
  const Defined *gp = ctx.es.riscvGlobalPointer;
  if (!gp)
    return;
...
static bool relax(Ctx &ctx, InputSection &sec) {
  const uint64_t secAddr = sec.getVA(ctx);
  ...
  for (auto [i, r] : llvm::enumerate(relocs)) {
    ...
    case R_RISCV_ALIGN: {
      ...
      if (LLVM_UNLIKELY(static_cast<int32_t>(remove) < 0)) {
        errorOrWarn(ctx, getErrorLocation(ctx, (const uint8_t *)loc) +
                             "insufficient padding bytes for " +
                             lld::toString(ctx, r.type) + ": " +
                             Twine(r.addend) +
                             " bytes available "
  ...
  if (!isUInt<32>(delta))
    ctx.fatal("section size decrease is too large: " + Twine(delta));
...
void RISCV::finalizeRelax(int passes) const {
  ...
  ctx.log("relaxation passes: " + Twine(passes));
  SmallVector<InputSection *, 0> storage;
  for (OutputSection *osec : ctx.outputSections) {
    if (!(osec->flags & SHF_EXECINSTR))
      continue;
...
static void mergeArch(Ctx &ctx, RISCVISAInfo::OrderedExtensionMap &mergedExts,
                      unsigned &mergedXlen, const InputSectionBase *sec,
                      StringRef s) {
  auto maybeInfo = RISCVISAInfo::parseNormalizedArchString(s);
  if (!maybeInfo) {
    errorOrWarn(ctx, toString(ctx, sec) + ": " + s + ": " +
...
mergeAttributesSection(Ctx &ctx,
  ...
  for (const InputSectionBase *sec : sections) {
    ...
    if (Error e = parser.parse(sec->content(), llvm::endianness::little))
      ctx.warn(toString(ctx, sec) + ": " + llvm::toString(std::move(e)));
      ...
      case RISCVAttrs::STACK_ALIGN:
        if (auto i = parser.getAttributeValue(tag.attr)) {
          ...
          if (r.second) {
            ...
            errorOrWarn(ctx,
                        toString(ctx, sec) + " has stack_align=" + Twine(*i) +
                            " but " + toString(ctx, firstStackAlign) +
  ...
  if (hasArch) {
    if (auto result = RISCVISAInfo::postProcessAndChecking(
      ...
      errorOrWarn(ctx, llvm::toString(result.takeError()));
    }
  }
...
TargetInfo *elf::getRISCVTargetInfo(Ctx &ctx) { return new RISCV(ctx); }
