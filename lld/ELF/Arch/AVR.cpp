...
TargetInfo *elf::getAVRTargetInfo(Ctx &ctx) { return new AVR(ctx); }

static uint32_t getEFlags(Ctx &ctx, InputFile *file) {
  return cast<ObjFile<ELF32LE>>(file)->getObj(ctx).getHeader().e_flags;
}

uint32_t AVR::calcEFlags() const {
  assert(!ctx.objectFiles.empty());

  uint32_t flags = getEFlags(ctx, ctx.objectFiles[0]);
  bool hasLinkRelaxFlag = flags & EF_AVR_LINKRELAX_PREPARED;

  for (InputFile *f : ArrayRef(ctx.objectFiles).slice(1)) {
    uint32_t objFlags = getEFlags(ctx, f);
