...
uint64_t elf::getPPC64TocBase(Ctx &ctx) {
  ...
  // the .got.
  uint64_t tocVA = ctx.in.got->getVA(ctx);
...
unsigned elf::getPPC64GlobalEntryToLocalEntryOffset(Ctx &ctx, uint8_t stOther) {
  ...
  ctx.error("reserved value of 7 in the 3 most-significant-bits of st_other");
  return 0;
}

void elf::writePrefixedInstruction(Ctx &ctx, uint8_t *loc, uint64_t insn) {
  insn = ctx.config->isLE ? insn << 32 | insn >> 32 : insn;
...
static void writeSequence(Ctx &ctx, MutableArrayRef<uint32_t> buf,
  ...
  auto *sec = ctx.make<InputSection>(
      ctx, nullptr, SHF_ALLOC, SHT_PROGBITS, 4,
...
void elf::addPPC64SaveRestore(Ctx &ctx) {
  ...
  writeSequence(ctx, ctx.restgpr0, "_restgpr0_", 14, 0xe9c1ff70,
                {0xe8010010, mtlr_0, blr});
  // _restgpr1_14: ld 14, -144(12); _restgpr1_15: ld 15, -136(12); ...
  // Tail: blr
  writeSequence(ctx, ctx.restgpr1, "_restgpr1_", 14, 0xe9ccff70, {blr});
  // _savegpr0_14: std 14, -144(1); _savegpr0_15: std 15, -136(1); ...
  // Tail: std 0, 16(1); blr
  writeSequence(ctx, ctx.savegpr0, "_savegpr0_", 14, 0xf9c1ff70,
                {0xf8010010, blr});
  // _savegpr1_14: std 14, -144(12); _savegpr1_15: std 15, -136(12); ...
  // Tail: blr
  writeSequence(ctx, ctx.savegpr1, "_savegpr1_", 14, 0xf9ccff70, {blr});
}

// Find the R_PPC64_ADDR64 in .rela.toc with matching offset.
template <typename ELFT>
static std::pair<Defined *, int64_t>
getRelaTocSymAndAddend(Ctx &ctx, InputSectionBase *tocSec, uint64_t offset) {
  ...
  for (;;) {
    if (relas[index].r_offset == offset) {
      Symbol &sym =
          tocSec->getFile<ELFT>()->getRelocTargetSym(ctx, relas[index]);
...
static bool tryRelaxPPC64TocIndirection(Ctx &ctx, const Relocation &rel,
  ...
  std::tie(d, addend) =
      ctx.config->isLE
          ? getRelaTocSymAndAddend<ELF64LE>(ctx, tocISB, rel.addend)
          : getRelaTocSymAndAddend<ELF64BE>(ctx, tocISB, rel.addend);
...
static void writeFromHalf16(Ctx &ctx, uint8_t *loc, uint32_t insn) {
  write32(ctx, ctx.config->isLE ? loc : loc - 2, insn);
}

static uint32_t readFromHalf16(Ctx &ctx, const uint8_t *loc) {
  return read32(ctx, ctx.config->isLE ? loc : loc - 2);
}

static uint64_t readPrefixedInstruction(Ctx &ctx, const uint8_t *loc) {
  uint64_t fullInstr = read64(ctx, loc);
  return ctx.config->isLE ? (fullInstr << 32 | fullInstr >> 32) : fullInstr;
}
...
static uint32_t getEFlags(Ctx &ctx, InputFile *file) {
  if (file->ekind == ELF64BEKind)
    return cast<ObjFile<ELF64BE>>(file)->getObj(ctx).getHeader().e_flags;
...
uint32_t PPC64::calcEFlags() const {
  for (InputFile *f : ctx.objectFiles) {
    uint32_t flag = getEFlags(ctx, f);
    if (flag == 1)
      ctx.error(toString(ctx, f) + ": ABI version 1 is not supported");
    else if (flag > 2)
      ctx.error(toString(ctx, f) + ": unrecognized e_flags: " + Twine(flag));
  }
  return 2;
}
...
void PPC64::relaxGot(uint8_t *loc, const Relocation &rel, uint64_t val) const {
  ...
  case R_PPC64_TOC16_LO_DS: {
    ...
    if (getPrimaryOpCode(insn) != LD)
      ctx.error("expected a 'ld' for got-indirect to toc-relative relaxing");
  ...
  case R_PPC64_GOT_PCREL34: {
    ...
    uint64_t insn = readPrefixedInstruction(ctx, loc);
    if ((insn & 0xfc000000) != 0xe4000000)
      ctx.error("expected a 'pld' for got-indirect to pc-relative relaxing");
    insn &= ~0xff000000fc000000;

    // Replace the cleared bits with the values for PADDI (0x600000038000000);
    insn |= 0x600000038000000;
    writePrefixedInstruction(ctx, loc, insn);
    relocate(loc, rel, val);
    break;
  }
  case R_PPC64_PCREL_OPT: {
    ...
    if (pcRelInsn == UINT64_C(-1)) {
      errorOrWarn(
          ctx, "unrecognized instruction for R_PPC64_PCREL_OPT relaxation: 0x" +
    ...
    // accessInsn to a nop.
    writePrefixedInstruction(ctx, loc,
...
void PPC64::relaxTlsGdToLe(uint8_t *loc, const Relocation &rel,
  ...
  case R_PPC64_GOT_TLSGD_PCREL34:
    ...
    writePrefixedInstruction(ctx, loc, 0x06000000386d0000);
  ...
  case R_PPC64_TLSGD: {
    ...
    if (locAsInt % 4 == 0) {
      ...
      // 2 bytes on BE.
      relocateNoSym(loc + 4 + (ctx.config->ekind == ELF64BEKind ? 2 : 0),
                    R_PPC64_TPREL16_LO, val);
    } else if (locAsInt % 4 == 1) {
      write32(ctx, loc - 1, NOP);
    } else {
      errorOrWarn(ctx, "R_PPC64_TLSGD has unexpected byte alignment");
    }
    break;
  }
  default:
...
void PPC64::relaxTlsLdToLe(uint8_t *loc, const Relocation &rel,
  ...
  case R_PPC64_GOT_TLSLD_PCREL34:
    ...
    writePrefixedInstruction(ctx, loc, 0x06000000386d1000);
    break;
  case R_PPC64_TLSLD: {
    // PC Relative Relaxation:
    ...
    if (locAsInt % 4 == 0) {
      ...
      errorOrWarn(ctx, "R_PPC64_TLSLD has unexpected byte alignment");
    }
    break;
  }
...
void PPC64::relaxTlsIeToLe(uint8_t *loc, const Relocation &rel,
  ...
  unsigned offset = (ctx.config->ekind == ELF64BEKind) ? 2 : 0;
  switch (rel.type) {
  ...
  case R_PPC64_GOT_TPREL_PCREL34: {
    const uint64_t pldRT =
        readPrefixedInstruction(ctx, loc) & 0x0000000003e00000;
    // paddi RT(from pld), r13, symbol@tprel, 0
    writePrefixedInstruction(ctx, loc, 0x06000000380d0000 | pldRT);
    relocateNoSym(loc, R_PPC64_TPREL34, val);
    break;
  }
  case R_PPC64_TLS: {
    ...
    if (locAsInt % 4 == 0) {
      ...
      if (primaryOp != 31)
        ctx.error("unrecognized instruction for IE to LE R_PPC64_TLS");
      ...
      if (primaryOp != 31)
        errorOrWarn(ctx, "unrecognized instruction for IE to LE R_PPC64_TLS");
      ...
      if (secondaryOp == 266) {
        ...
        if (dFormOp == 0) { // Expecting a DS-Form instruction.
          ...
          if (dFormOp == 0)
            errorOrWarn(ctx,
                        "unrecognized instruction for IE to LE R_PPC64_TLS");
        }
        write32(ctx, loc - 1, (dFormOp | (tlsInstr & 0x03ff0000)));
      }
    } else {
      errorOrWarn(ctx, "R_PPC64_TLS must be either 4 byte aligned or one byte "
...
RelExpr PPC64::getRelExpr(RelType type, const Symbol &s,
  ...
  case R_PPC64_TOC16_HA:
  case R_PPC64_TOC16_LO_DS:
    return ctx.config->tocOptimize ? R_PPC64_RELAX_TOC : R_GOTREL;
  case R_PPC64_TOC:
...
void PPC64::writePltHeader(uint8_t *buf) const {
  ...
  int64_t gotPltOffset =
      ctx.in.gotPlt->getVA(ctx) - (ctx.in.plt->getVA(ctx) + 8);
...
void PPC64::relocate(uint8_t *loc, const Relocation &rel, uint64_t val) const {
  ...
  case R_PPC64_REL16_HA:
  case R_PPC64_TPREL16_HA:
    if (ctx.config->tocOptimize && shouldTocOptimize && ha(val) == 0)
      writeFromHalf16(ctx, loc, NOP);
    else {
  ...
  case R_PPC64_TPREL34: {
    ...
    uint64_t instr = readPrefixedInstruction(ctx, loc) & ~fullMask;
    writePrefixedInstruction(ctx, loc,
...
RelExpr PPC64::adjustGotPcExpr(RelType type, int64_t addend,
  ...
  if ((type == R_PPC64_GOT_PCREL34 || type == R_PPC64_PCREL_OPT) &&
    ...
    if ((readPrefixedInstruction(ctx, loc) & 0xfc000000) == 0xe4000000)
...
void PPC64::relaxTlsGdToIe(uint8_t *loc, const Relocation &rel,
  ...
  case R_PPC64_GOT_TLSGD_PCREL34: {
    ...
    writePrefixedInstruction(ctx, loc, 0x04100000e4600000);
    ...
    case R_PPC64_CALL:
      ...
      if (rel.sym->needsTocRestore()) {
        ...
        if ((rel.offset + 8 > sec.content().size() ||
          ...
          errorOrWarn(ctx, getErrorLocation(ctx, loc) + "call to " +
                               lld::toString(ctx, *rel.sym).substr(6) +
...
bool PPC64::adjustPrologueForCrossSplitStack(uint8_t *loc, uint8_t *end,
  ...
  // instructions.
  if (stackFrameSize < ctx.config->splitStackAdjustSize + INT32_MIN) {
    ctx.error(getErrorLocation(ctx, loc) +
  ...
  int32_t adjustedStackFrameSize =
      stackFrameSize - ctx.config->splitStackAdjustSize;
...
TargetInfo *elf::getPPC64TargetInfo(Ctx &ctx) { return new PPC64(ctx); }
