...
void X86::writePltHeader(uint8_t *buf) const {
  ...
  uint32_t gotPlt = ctx.in.gotPlt->getVA(ctx);
...
void X86::writePlt(uint8_t *buf, const Symbol &sym,
  ...
  if (ctx.config->isPic) {
    ...
    write32le(buf + 2, sym.getGotPltVA(ctx) - ctx.in.gotPlt->getVA(ctx));
  } else {
    const uint8_t inst[] = {
...
static void relaxTlsGdToLe(Ctx &ctx, uint8_t *loc, const Relocation &rel,
  ...
  if (rel.type == R_386_TLS_GD) {
    ...
    if (memcmp(loc - 2, "\x8d\x83", 2)) {
      ctx.error(getErrorLocation(ctx, loc - 2) +
...
IntelIBT::IntelIBT(Ctx &ctx) : X86(ctx) { pltHeaderSize = 0; }
...
void IntelIBT::writePlt(uint8_t *buf, const Symbol &sym,
  ...
  if (ctx.config->isPic) {
    ...
    memcpy(buf, inst, sizeof(inst));
    write32le(buf + 6, sym.getGotPltVA(ctx) - ctx.in.gotPlt->getVA(ctx));
    return;
  }
...
void RetpolinePic::writePlt(uint8_t *buf, const Symbol &sym,
  ...
  uint32_t ebx = ctx.in.gotPlt->getVA(ctx);
  unsigned off = pltEntryAddr - ctx.in.plt->getVA(ctx);
...
TargetInfo *elf::getX86TargetInfo(Ctx &ctx) {
  if (ctx.config->zRetpolineplt) {
    if (ctx.config->isPic) {
      return new RetpolinePic(ctx);
    }
    return new RetpolineNoPic(ctx);
  }
