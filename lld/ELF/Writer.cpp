...
static bool needsInterpSection(Ctx &ctx) {
  return !ctx.config->relocatable && !ctx.config->shared &&
         !ctx.config->dynamicLinker.empty() && ctx.script->needsInterpSection();
}
  ...
  for (unsigned part = 2; part != ctx.partitions.size() + 1; ++part) {
    for (InputSectionBase *s : ctx.inputSections) {
      if (!(s->flags & SHF_ALLOC) || !s->isLive() || s->type != SHT_NOTE)
        continue;
      auto *copy = ctx.make<InputSection>(cast<InputSection>(*s));
      copy->partition = part;
      newSections.push_back(copy);
    }
    for (size_t i = 0; i != ehSize; ++i) {
      assert(ctx.ehInputSections[i]->isLive());
      auto *copy = ctx.make<EhInputSection>(*ctx.ehInputSections[i]);
      copy->partition = part;
...
void elf::addReservedSymbols(Ctx &ctx) {
  if (ctx.config->emachine == EM_MIPS) {
    auto addAbsolute = [&ctx](StringRef name) {
      Symbol *sym =
    ...
    ctx.es.mipsGp = addAbsolute("_gp");

    // On MIPS O32 ABI, _gp_disp is a magic symbol designates offset between
    // start of function and 'gp' pointer into GOT.
    if (ctx.symtab.find("_gp_disp"))
      ctx.es.mipsGpDisp = addAbsolute("_gp_disp");
    ...
    if (ctx.symtab.find("__gnu_local_gp"))
      ctx.es.mipsLocalGp = addAbsolute("__gnu_local_gp");
  } else if (ctx.config->emachine == EM_PPC) {
  ...
  StringRef gotSymName =
      (ctx.config->emachine == EM_PPC64) ? ".TOC." : "_GLOBAL_OFFSET_TABLE_";

  if (Symbol *s = ctx.symtab.find(gotSymName)) {
    if (s->isDefined()) {
      ctx.error(toString(ctx, s->file) +
                " cannot redefine linker defined symbol '" + gotSymName + "'");
      return;
    }
    ...
    s->resolve(ctx, Defined{ctx, ctx.internalFile, StringRef(), STB_GLOBAL,
                            ...
                            ctx.out.elfHeader});
    ctx.es.globalOffsetTable = cast<Defined>(s);
  }
  ...
  auto add = [&ctx](StringRef s, int64_t pos) {
    return addOptionalRegular(ctx, s, ctx.out.elfHeader, pos, STV_DEFAULT);
  };

  ctx.es.bss = add("__bss_start", 0);
  ctx.es.end1 = add("end", -1);
  ctx.es.end2 = add("_end", -1);
  ctx.es.etext1 = add("etext", -1);
  ctx.es.etext2 = add("_etext", -1);
  ctx.es.edata1 = add("edata", -1);
  ctx.es.edata2 = add("_edata", -1);
}

static void demoteDefined(Ctx &ctx, Defined &sym,
  ...
  for (Symbol *sym : ctx.symtab.getSymbols()) {
    if (auto *d = dyn_cast<Defined>(sym)) {
      if (d->section && !d->section->isLive())
        demoteDefined(ctx, *d, sectionIndexMap[d->file]);
    } else {
    ...
    if (ctx.config->hasDynSymTab)
      sym->isPreemptible = computeIsPreemptible(ctx, *sym);
  }
}

bool elf::hasMemtag(Ctx &ctx) {
  return ctx.config->emachine == EM_AARCH64 &&
         ctx.config->androidMemtagMode != ELF::NT_MEMTAG_LEVEL_NONE;
}
...
bool elf::canHaveMemtagGlobals(Ctx &ctx) {
  return hasMemtag(ctx) && (ctx.config->relocatable || ctx.config->shared ||
...
template <class ELFT> void elf::createSyntheticSections(Ctx &ctx) {
  ...
  ctx.out.tlsPhdr = nullptr;
  ctx.out.preinitArray = nullptr;
  ctx.out.initArray = nullptr;
  ctx.out.finiArray = nullptr;
  ...
  auto add = [&ctx](SyntheticSection &sec) {
    ctx.inputSections.push_back(&sec);
  };
  ...
  ctx.out.programHeaders = ctx.make<OutputSection>(ctx, "", 0, SHF_ALLOC);
  ctx.out.programHeaders->addralign = ctx.config->wordsize;

  if (ctx.config->strip != StripPolicy::All) {
  ...
  if (ctx.config->emachine == EM_MIPS) {
    if (!ctx.config->shared && ctx.config->hasDynSymTab) {
  ...
  StringRef relaDynName = ctx.config->isRela ? ".rela.dyn" : ".rel.dyn";

  const unsigned threadCount = ctx.config->threadCount;
  for (Partition &part : ctx.partitions) {
    ...
    if (ctx.config->buildId != BuildIdKind::None) {
    ...
    if (!ctx.config->packageMetadata.empty()) {
  ...
  if (ctx.partitions.size() != 1) {
    ...
    ctx.in.partEnd = std::make_unique<BssSection>(ctx, ".part.end",
                                                  ctx.config->maxPageSize, 1);
  ...
  if (ctx.es.globalOffsetTable && ctx.config->emachine != EM_MIPS) {
  ...
  if (ctx.config->gdbIndex)
    add(*GdbIndexSection::create<ELFT>(ctx));

  // We always need to add rel[a].plt to output if it has entries.
  // Even for static linking it can contain R_[*]_IRELATIVE relocations.
  ctx.in.relaPlt = std::make_unique<RelocationSection<ELFT>>(
      ctx, ctx.config->isRela ? ".rela.plt" : ".rel.plt", /*sort=*/false,
      /*threadCount=*/1);
  add(*ctx.in.relaPlt);
  ...
  ctx.in.relaIplt = std::make_unique<RelocationSection<ELFT>>(
      ctx,
      ctx.config->androidPackDynRelocs ? ctx.in.relaPlt->name : relaDynName,
  ...
  if (ctx.config->andFeatures)
    add(*ctx.make<GnuPropertySection>(ctx));
  ...
  if (ctx.config->relocatable)
    add(*ctx.make<GnuStackSection>(ctx));
...
template <class ELFT> void Writer<ELFT>::run() {
  ...
  if (ctx.config->printMemoryUsage)
    ctx.script->printMemoryUsage(ctx.outs());

  if (ctx.config->checkSections)
    checkSections();
...

  {
    ...
    if (!ctx.config->oFormatBinary) {
      if (ctx.config->zSeparate != SeparateSegmentKind::None)
        writeTrapInstr();
      writeHeader();
      writeSections();
    } else {
...
static void markUsedLocalSymbolsImpl(Ctx &ctx, ObjFile<ELFT> *file,
                                     llvm::ArrayRef<RelTy> rels) {
  for (const RelTy &rel : rels) {
    Symbol &sym = file->getRelocTargetSym(ctx, rel);
    if (sym.isLocal())
      sym.used = true;
  }
}
  ...
  for (ELFFileBase *file : ctx.objectFiles) {
    ...
    for (InputSectionBase *s : f->getSections()) {
      ...
      if (isec->type == SHT_REL)
        markUsedLocalSymbolsImpl(ctx, f, isec->getDataAs<typename ELFT::Rel>());
      else if (isec->type == SHT_RELA)
        markUsedLocalSymbolsImpl(ctx, f,
                                 isec->getDataAs<typename ELFT::Rela>());
    }
  }
}
...
static bool shouldKeepInSymtab(Ctx &ctx, const Defined &sym) {
  ...
  // from live sections.
  if (sym.used && ctx.config->copyRelocs)
    return true;
  ...
  if (ctx.config->emachine == EM_ARM && sym.section &&
      sym.section->type == SHT_ARM_EXIDX)
    return false;

  if (ctx.config->discard == DiscardPolicy::None)
    return true;
  if (ctx.config->discard == DiscardPolicy::All)
    return false;
  ...
  if (sym.getName().starts_with(".L") &&
      (ctx.config->discard == DiscardPolicy::Locals ||
       (sym.section && (sym.section->flags & SHF_MERGE))))
    return false;
  return true;
}

bool lld::elf::includeInSymtab(Ctx &ctx, const Symbol &b) {
  if (auto *d = dyn_cast<Defined>(&b)) {
    ...
    if (auto *s = dyn_cast<MergeInputSection>(sec))
      return s->getSectionPiece(ctx, d->value).live;
    return true;
  }
  return b.used || !ctx.config->gcSections;
}
  ...
  for (ELFFileBase *file : ctx.objectFiles) {
    ...
    for (Symbol *b : file->getLocalSymbols()) {
      ...
      if (dr->section && !dr->section->isLive())
        demoteDefined(ctx, *dr, sectionIndexMap);
...
static unsigned getSectionRank(Ctx &ctx, OutputSection &osec) {
  unsigned rank = osec.partition * RF_PARTITION;

  // We want to put section specified by -T option first, so we
  // can start assigning VA starting from them later.
  if (ctx.config->sectionStartMap.count(osec.name))
    return rank;
  ...
  if (!isWrite && !isExec) {
    ...
    if (!(osec.flags & SHF_X86_64_LARGE && ctx.config->emachine == EM_X86_64))
      rank |= RF_LARGE;
  } else if (isExec) {
...
static bool compareSections(Ctx &ctx, const SectionCommand *aCmd,
  ...
  if (!(a->sortRank & RF_NOT_ADDR_SET))
    return ctx.config->sectionStartMap.lookup(a->name) <
           ctx.config->sectionStartMap.lookup(b->name);
  return false;
}

PhdrEntry::PhdrEntry(Ctx &ctx, unsigned type, unsigned flags)
    : p_align(type == llvm::ELF::PT_LOAD ? ctx.config->maxPageSize : 0),
...
template <class ELFT> void Writer<ELFT>::addRelIpltSymbols() {
  ...
  // We'll override ctx.out.elfHeader with In.relaIplt later when we are
  // sure that .rela.plt exists in output.
  ctx.es.relaIpltStart = addOptionalRegular(
      ctx, ctx.config->isRela ? "__rela_iplt_start" : "__rel_iplt_start",
      ctx.out.elfHeader, 0, STV_HIDDEN);

  ctx.es.relaIpltEnd = addOptionalRegular(
      ctx, ctx.config->isRela ? "__rela_iplt_end" : "__rel_iplt_end",
      ctx.out.elfHeader, 0, STV_HIDDEN);
}
...
template <class ELFT> void Writer<ELFT>::setReservedSymbolSections() {
  if (ctx.es.globalOffsetTable) {
    // The _GLOBAL_OFFSET_TABLE_ symbol is defined by ctx.target convention
    // usually to the start of the .got or .got.plt section.
    ...
    ctx.es.globalOffsetTable->section = sec;
  }

  // .rela_iplt_{start,end} mark the start and the end of in.relaIplt.
  if (ctx.es.relaIpltStart && ctx.in.relaIplt->isNeeded(ctx)) {
    ctx.es.relaIpltStart->section = ctx.in.relaIplt.get();
    ctx.es.relaIpltEnd->section = ctx.in.relaIplt.get();
    ctx.es.relaIpltEnd->value = ctx.in.relaIplt->getSize(ctx);
  }
  ...
  if (lastRO) {
    ...
    if (ctx.es.etext1)
      ctx.es.etext1->section = lastRO->lastSec;
    if (ctx.es.etext2)
      ctx.es.etext2->section = lastRO->lastSec;
  }
  ...
  if (last) {
    ...
    if (ctx.es.edata1)
      ctx.es.edata1->section = edata;
    if (ctx.es.edata2)
      ctx.es.edata2->section = edata;

    // _end is the first location after the uninitialized data region.
    if (ctx.es.end1)
      ctx.es.end1->section = last->lastSec;
    if (ctx.es.end2)
      ctx.es.end2->section = last->lastSec;
  }

  if (ctx.es.bss) {
    // On RISC-V, set __bss_start to the start of .sbss if present.
    OutputSection *sbss =
        ctx.config->emachine == EM_RISCV ? findSection(ctx, ".sbss") : nullptr;
    ctx.es.bss->section = sbss ? sbss : findSection(ctx, ".bss");
  }
  ...
  if (ctx.es.mipsGp) {
    ...
    for (OutputSection *os : ctx.outputSections) {
      if (os->flags & SHF_MIPS_GPREL) {
        ctx.es.mipsGp->section = os;
        ctx.es.mipsGp->value = 0x7ff0;
        break;
      }
    }
  }
...
findOrphanPos(Ctx &ctx, SmallVectorImpl<SectionCommand *>::iterator b,
  ...
  if (ctx.script->hasPhdrsCommands() || !ctx.script->memoryRegions.empty())
...
static DenseMap<const InputSectionBase *, int> buildSectionOrder(Ctx &ctx) {
  ...
  if (ctx.config->symbolOrderingFile.empty())
    return sectionOrder;
  ...
  int priority = -ctx.config->symbolOrderingFile.size();
  for (StringRef s : ctx.config->symbolOrderingFile)
  ...
  if (ctx.config->warnSymbolOrdering)
    for (auto orderEntry : symbolOrder)
      if (!orderEntry.second.present)
        ctx.warn("symbol ordering file: no such symbol: " +
...
sortISDBySectionOrder(Ctx &ctx, InputSectionDescription *isd,
  ...
  for (InputSection *isec : isd->sections) {
    if (executableOutputSection)
      totalSize += isec->getSize(ctx);
    auto i = order.find(isec);
    if (i == order.end()) {
      unorderedSections.push_back(isec);
      unorderedSize += isec->getSize(ctx);
      continue;
    }
  ...
  if (executableOutputSection && !orderedSections.empty() &&
    ...
    for (; insPt != unorderedSections.size(); ++insPt) {
      unorderedPos += unorderedSections[insPt]->getSize(ctx);
...
static void sortSection(Ctx &ctx, OutputSection &osec,
  ...
  if (ctx.in.relaIplt->getParent() == &osec)
    return;
  ...
  if (name == ".init_array" || name == ".fini_array") {
    ...
    osec.sortCtorsDtors();
  } else if (ctx.config->emachine == EM_PPC64 && name == ".toc") {
...
template <class ELFT> void Writer<ELFT>::sortSections() {
  ...
  if (!ctx.script->hasSectionsCommand) {
    ...
    std::stable_sort(
        ...
        [&ctx = ctx](const SectionCommand *aCmd, const SectionCommand *bCmd) {
          return compareSections(ctx, aCmd, bCmd);
        });
  }
...
template <class ELFT> void Writer<ELFT>::resolveShfLinkOrder() {
  ...
  for (OutputSection *sec : ctx.outputSections) {
    ...
    if (!ctx.config->relocatable && ctx.config->emachine == EM_ARM &&
    ...
    for (SectionCommand *cmd : sec->commands) {
      ...
      if (hasLinkOrder && ctx.errorCount() == 0) {
...
template <class ELFT> void Writer<ELFT>::finalizeAddressDependentContent() {
  ...
  if (ctx.config->emachine == EM_HEXAGON)
    hexagonTLSSymbolUpdate(ctx, ctx.outputSections);
  ...
  for (;;) {
    ...
    if (changed && pass >= 30) {
      ctx.error(ctx.target->needsThunks ? "thunk creation not converged"
    ...
    const Defined *changedSym = ctx.script->assignAddresses();
    if (!changed) {
      ...
      if (!changedSym)
        break;
      if (++assignPasses == 5) {
        errorOrWarn(ctx, "assignment to symbol " + toString(ctx, *changedSym) +
                             " does not converge");
        break;
      }
    }
  ...
  for (SectionCommand *cmd : ctx.script->sectionCommands)
    if (auto *osd = dyn_cast<OutputDesc>(cmd)) {
      ...
      if (osec->addr % osec->addralign != 0)
        ctx.warn("address (0x" + Twine::utohexstr(osec->addr) +
                 ") of section " + osec->name +
                 " is not a multiple of alignment (" + Twine(osec->addralign) +
                 ")");
    }
}
...
template <class ELFT> void Writer<ELFT>::optimizeBasicBlockJumps() {
  assert(ctx.config->optimizeBBJumps);
...
template <class ELFT> void Writer<ELFT>::finalizeSections() {
  if (!ctx.config->relocatable) {
    ...
    if (ctx.config->emachine == EM_RISCV) {
      ctx.es.riscvGlobalPointer = nullptr;
      if (!ctx.config->shared) {
        ...
        // relaxation.
        if (ctx.config->relaxGP) {
          Symbol *s = ctx.symtab.find("__global_pointer$");
          if (s && s->isDefined())
            ctx.es.riscvGlobalPointer = cast<Defined>(s);
        }
      }
    }
    ...
    if (ctx.config->emachine == EM_386 || ctx.config->emachine == EM_X86_64) {
      ...
      if (s && s->isUndefined()) {
        s->resolve(ctx, Defined{ctx, ctx.internalFile, StringRef(), STB_GLOBAL,
                                ...
                                /*section=*/nullptr});
        ctx.es.tlsModuleBase = cast<Defined>(s);
      }
    }
  ...
  if (ctx.config->copyRelocs && ctx.config->discard != DiscardPolicy::None)
    markUsedLocalSymbols<ELFT>(ctx);
  demoteAndCopyLocalSymbols(ctx);

  if (ctx.config->copyRelocs)
    addSectionSymbols();
  ...
  if (!ctx.config->relocatable) {
    ...
    ctx.ppc64noTocRelax.clear();
    scanRelocations<ELFT>(ctx);
    reportUndefinedSymbols(ctx);
    postScanRelocations(ctx);

    if (ctx.in.plt && ctx.in.plt->isNeeded(ctx))
      ctx.in.plt->addSymbols(ctx);
    if (ctx.in.iplt && ctx.in.iplt->isNeeded(ctx))
      ctx.in.iplt->addSymbols(ctx);

    if (ctx.config->unresolvedSymbolsInShlib != UnresolvedPolicy::Ignore) {
      auto diagnose =
          ctx.config->unresolvedSymbolsInShlib == UnresolvedPolicy::ReportError
              ? errorOrWarn
              : [](Ctx &ctx, const Twine &msg) { ctx.warn(msg); };
      ...
      for (SharedFile *file : ctx.sharedFiles) {
        ...
        for (Symbol *sym : file->requiredSymbols) {
          ...
          if (sym->isUndefined() && !sym->isWeak()) {
            ...
            diagnose(ctx, "non-exported symbol '" + toString(ctx, *sym) +
                              "' in '" + toString(ctx, sym->file) +
                              "' is referenced by DSO '" + toString(ctx, file) +
                              "'");
          }
        }
      }
  ...
  for (OutputSection *sec : ctx.outputSections) {
    auto i = ctx.config->sectionStartMap.find(sec->name);
    if (i != ctx.config->sectionStartMap.end())
  ...
  if (!ctx.config->relocatable && !ctx.config->oFormatBinary) {
    for (Partition &part : ctx.partitions) {
...

  {
    ...
    finalizeSynthetic(ctx, ctx.in.relaIplt.get());
    ...
    for (Partition &part : ctx.partitions) {
      if (part.relaDyn) {
        ...
        part.relaDyn->partitionRels(ctx);
  ...
  if (!ctx.script->hasSectionsCommand && !ctx.config->relocatable)
    ...
    if (osec->flags & SHF_EXECINSTR)
      for (InputSection *isec : getInputSections(*osec, storage))
        if (!(isec->flags & SHF_EXECINSTR))
          ctx.error(
              "cannot place " + toString(ctx, isec) + " into " +
              toString(osec->name) +
...
template <class ELFT> void Writer<ELFT>::addStartEndSymbols() {
  ...
  OutputSection *Default = findSection(ctx, ".text");
  if (!Default)
    Default = ctx.out.elfHeader;

  auto define = [=](StringRef start, StringRef end, OutputSection *os) {
    if (os && !ctx.script->isDiscarded(os)) {
      addOptionalRegular(ctx, start, os, 0);
      addOptionalRegular(ctx, end, os, -1);
    } else {
...
void Writer<ELFT>::addStartStopSymbols(OutputSection &osec) {
  ...
  if (!isValidCIdentifier(s))
    return;
  addOptionalRegular(ctx, ctx.saver.save("__start_" + s), &osec, 0,
                     ctx.config->zStartStopVisibility);
  addOptionalRegular(ctx, ctx.saver.save("__stop_" + s), &osec, -1,
                     ctx.config->zStartStopVisibility);
}
...
static uint64_t computeFlags(Ctx &ctx, uint64_t flags) {
  if (ctx.config->omagic)
    return PF_R | PF_W | PF_X;
  if (ctx.config->executeOnly && (flags & PF_X))
    return flags & ~PF_R;
  if (ctx.config->singleRoRx && !(flags & PF_W))
    return flags | PF_X;
  return flags;
}
...
SmallVector<PhdrEntry *, 0> Writer<ELFT>::createPhdrs(Partition &part) {
  ...
  auto addHdr = [&](unsigned type, unsigned flags) -> PhdrEntry * {
    ret.push_back(ctx.make<PhdrEntry>(ctx, type, flags));
    return ret.back();
  };
  ...
  if (!ctx.config->nmagic && !ctx.config->omagic) {
    ...
    if (OutputSection *cmd = findSection(ctx, ".interp", partNo))
      addHdr(PT_INTERP, cmd->getPhdrFlags(ctx))->add(cmd);
  ...
  PhdrEntry *relRo = ctx.make<PhdrEntry>(ctx, PT_GNU_RELRO, PF_R);
  ...
  for (OutputSection *sec : ctx.outputSections) {
    ...
    if (sec->partition != partNo) {
      if (isMain && sec->partition == 255)
        addHdr(PT_LOAD, computeFlags(ctx, sec->getPhdrFlags(ctx)))->add(sec);
      continue;
    }
    ...
    uint64_t newFlags = computeFlags(ctx, sec->getPhdrFlags(ctx));
    bool sameLMARegion =
  ...
  PhdrEntry *tlsHdr = ctx.make<PhdrEntry>(ctx, PT_TLS, PF_R);
  for (OutputSection *sec : ctx.outputSections)
  ...
  if (OutputSection *sec = part.dynamic->getParent())
    addHdr(PT_DYNAMIC, sec->getPhdrFlags(ctx))->add(sec);
  ...
  if (part.ehFrame->isNeeded(ctx) && part.ehFrameHdr &&
    ...
    addHdr(PT_GNU_EH_FRAME, part.ehFrameHdr->getParent()->getPhdrFlags(ctx))
        ->add(part.ehFrameHdr->getParent());

  // PT_OPENBSD_RANDOMIZE is an OpenBSD-specific feature. That makes
  // the dynamic linker fill the segment with random data.
  if (OutputSection *cmd = findSection(ctx, ".openbsd.randomdata", partNo))
    addHdr(PT_OPENBSD_RANDOMIZE, cmd->getPhdrFlags(ctx))->add(cmd);

  if (ctx.config->zGnustack != GnuStackKind::None) {
    // PT_GNU_STACK is a special section to tell the loader to make the
    // pages for the stack non-executable. If you really want an executable
    // stack, you can pass -z execstack, but that's not recommended for
    // security reasons.
    unsigned perm = PF_R | PF_W;
    if (ctx.config->zGnustack == GnuStackKind::Exec)
      perm |= PF_X;
    addHdr(PT_GNU_STACK, perm)->p_memsz = ctx.config->zStackSize;
  }
...
void Writer<ELFT>::addPhdrForSection(Partition &part, unsigned shType,
  ...
  PhdrEntry *entry = ctx.make<PhdrEntry>(ctx, pType, pFlags);
  entry->add(*i);
...
template <class ELFT> void Writer<ELFT>::fixSectionAlignments() {
  ...
  auto pageAlign = [&](const PhdrEntry *p) {
    ...
    if (!cmd->addrExpr) {
      ...
      if (ctx.config->zSeparate == SeparateSegmentKind::Loadable ||
          (ctx.config->zSeparate == SeparateSegmentKind::Code && prev &&
           (prev->p_flags & PF_X) != (p->p_flags & PF_X)) ||
          cmd->type == SHT_LLVM_PART_EHDR)
        cmd->addrExpr = [&ctx = ctx] {
          return alignToPowerOf2(ctx.script->getDot(), ctx.config->maxPageSize);
        };
      ...
      else if (ctx.out.tlsPhdr && ctx.out.tlsPhdr->firstSec == p->firstSec)
        cmd->addrExpr = [&ctx = ctx] {
          return alignToPowerOf2(ctx.script->getDot(),
                                 ctx.config->maxPageSize) +
                 alignToPowerOf2(ctx.script->getDot() % ctx.config->maxPageSize,
                                 ctx.out.tlsPhdr->p_align);
        };
      else
        cmd->addrExpr = [&ctx = ctx] {
          return alignToPowerOf2(ctx.script->getDot(),
                                 ctx.config->maxPageSize) +
                 ctx.script->getDot() % ctx.config->maxPageSize;
        };
    }
  };
...
static uint64_t computeFileOffset(Ctx &ctx, OutputSection *os, uint64_t off) {
  ...
  if (os->type == SHT_NOBITS &&
      (!ctx.out.tlsPhdr || ctx.out.tlsPhdr->firstSec != os))
    return off;
...
template <class ELFT> void Writer<ELFT>::assignFileOffsets() {
  ...
  for (OutputSection *sec : ctx.outputSections) {
    ...
    if (ctx.config->zSeparate != SeparateSegmentKind::None && lastRX &&
        lastRX->lastSec == sec)
      off = alignToPowerOf2(off, ctx.config->maxPageSize);
  }
  for (OutputSection *osec : ctx.outputSections)
    if (!(osec->flags & SHF_ALLOC)) {
  ...
  sectionHeaderOff = alignToPowerOf2(off, ctx.config->wordsize);
  fileSize =
  ...
  for (OutputSection *sec : ctx.outputSections) {
    ...
    if ((sec->offset > fileSize) || (sec->offset + sec->size > fileSize))
      ctx.error("unable to place section " + sec->name + " at file offset " +
...
template <class ELFT> void Writer<ELFT>::setPhdrs(Partition &part) {
  for (PhdrEntry *p : part.phdrs) {
    ...
    if (part.armExidx && p->p_type == PT_ARM_EXIDX) {
      ...
      p->p_memsz = part.armExidx->getSize(ctx);
...
static void checkOverlap(Ctx &ctx, StringRef name,
  ...
  for (size_t i = 1, end = sections.size(); i < end; ++i) {
    ...
    errorOrWarn(
        ctx, "section " + a.sec->name + " " + name + " range overlaps with " +
                 b.sec->name + "\n>>> " + a.sec->name + " range is " +
...
template <class ELFT> void Writer<ELFT>::checkSections() {
  ...
  for (OutputSection *os : ctx.outputSections)
    if ((os->addr + os->size < os->addr) ||
      ...
      errorOrWarn(ctx, "section " + os->name + " at 0x" + utohexstr(os->addr) +
  ...
  for (OutputSection *sec : ctx.outputSections)
    if (sec->size > 0 && sec->type != SHT_NOBITS &&
        (!ctx.config->oFormatBinary || (sec->flags & SHF_ALLOC)))
...
static uint64_t getEntryAddr(Ctx &ctx) {
  // Case 1, 2 or 3
  if (Symbol *b = ctx.symtab.find(ctx.config->entry))
    return b->getVA(ctx);

  // Case 4
  uint64_t addr;
  if (to_integer(ctx.config->entry, addr))
    return addr;

  // Case 5
  if (ctx.config->warnMissingEntry)
    ctx.warn("cannot find entry symbol " + ctx.config->entry +
...
template <class ELFT> void Writer<ELFT>::writeHeader() {
  writeEhdr<ELFT>(ctx, ctx.out.bufferStart, *ctx.mainPart);
  writePhdrs<ELFT>(ctx.out.bufferStart + sizeof(Elf_Ehdr), *ctx.mainPart);

  auto *eHdr = reinterpret_cast<Elf_Ehdr *>(ctx.out.bufferStart);
  ...
  auto *sHdrs =
      reinterpret_cast<Elf_Shdr *>(ctx.out.bufferStart + eHdr->e_shoff);
...
template <class ELFT> void Writer<ELFT>::openFile() {
  uint64_t maxSize = ctx.config->is64 ? INT64_MAX : UINT32_MAX;
  ...
  unlinkAsync(ctx.config->outputFile);
  unsigned flags = 0;
  if (!ctx.config->relocatable)
    flags |= FileOutputBuffer::F_executable;
  if (!ctx.config->mmapOutputFile)
    flags |= FileOutputBuffer::F_no_mmap;
  Expected<std::unique_ptr<FileOutputBuffer>> bufferOrErr =
      FileOutputBuffer::create(ctx.config->outputFile, fileSize, flags);
  ...
  ctx.out.bufferStart = buffer->getBufferStart();
}
...
template <class ELFT> void Writer<ELFT>::writeSectionsBinary() {
  ...
  for (OutputSection *sec : ctx.outputSections)
    if (sec->flags & SHF_ALLOC)
      sec->writeTo<ELFT>(ctx, ctx.out.bufferStart + sec->offset, tg);
}

static void fillTrap(Ctx &ctx, uint8_t *i, uint8_t *end) {
  for (; i + 4 <= end; i += 4)
  ...
  for (Partition &part : ctx.partitions) {
    ...
    if (last && (last->p_flags & PF_X))
      last->p_memsz = last->p_filesz =
          alignToPowerOf2(last->p_filesz, ctx.config->maxPageSize);
  }
}
...
template <class ELFT> void Writer<ELFT>::writeSections() {
  ...
  if (ctx.config->checkDynamicRelocs && ctx.config->writeAddends) {
    for (OutputSection *sec : ctx.outputSections)
      if (sec->type == SHT_REL || sec->type == SHT_RELA)
        sec->checkDynRelAddends(ctx, ctx.out.bufferStart);
  }
}
...
template <class ELFT> void Writer<ELFT>::writeBuildId() {
  ...
  if (ctx.config->buildId == BuildIdKind::Hexstring) {
    for (Partition &part : ctx.partitions)
      part.buildId->writeBuildId(ctx.config->buildIdVector);
    return;
  }
  ...
  llvm::ArrayRef<uint8_t> input{ctx.out.bufferStart, size_t(fileSize)};
...
template void elf::createSyntheticSections<ELF32LE>(Ctx &ctx);
template void elf::createSyntheticSections<ELF32BE>(Ctx &ctx);
template void elf::createSyntheticSections<ELF64LE>(Ctx &ctx);
template void elf::createSyntheticSections<ELF64BE>(Ctx &ctx);

template void elf::writeResult<ELF32LE>(Ctx &ctx);
template void elf::writeResult<ELF32BE>(Ctx &ctx);
template void elf::writeResult<ELF64LE>(Ctx &ctx);
template void elf::writeResult<ELF64BE>(Ctx &ctx);
