...
static uint64_t scanCortexA53Errata843419(Ctx &ctx, InputSection *isec,
...
public:
  Patch843419Section(Ctx &ctx, InputSection *p, uint64_t off);

  void writeTo(Ctx &ctx, uint8_t *buf) override;

  size_t getSize(Ctx &ctx) const override { return 8; }

  uint64_t getLDSTAddr(Ctx &ctx) const;
...
Patch843419Section::Patch843419Section(Ctx &ctx, InputSection *p, uint64_t off)
  ...
  patchSym = addSyntheticLocal(
      ctx, ctx.saver.save("__CortexA53843419_" + utohexstr(getLDSTAddr(ctx))),
      STT_FUNC, 0, getSize(ctx), *this);
  addSyntheticLocal(ctx, ctx.saver.save("$x"), STT_NOTYPE, 0, 0, *this);
}

uint64_t Patch843419Section::getLDSTAddr(Ctx &ctx) const {
  ...
  for (const InputSection *isec : isd.sections) {
    ...
    if (isecLimit > patchUpperBound) {
      while (patchIt != patchEnd) {
        if ((*patchIt)->getLDSTAddr(ctx) - outSecAddr >= prevIsecLimit)
          break;
...
static void implementPatch(Ctx &ctx, uint64_t adrpAddr, uint64_t patcheeOffset,
  ...
  ctx.log("detected cortex-a53-843419 erratum sequence starting at " +
          utohexstr(adrpAddr) + " in unpatched output.");

  auto *ps = ctx.make<Patch843419Section>(ctx, isec, patcheeOffset);
...
AArch64Err843419Patcher::patchInputSectionDescription(
  ...
  for (InputSection *isec : isd.sections) {
    ...
    while (codeSym != mapSyms.end()) {
      ...
      while (off < limit) {
        uint64_t startAddr = isec->getVA(ctx, off);
        if (uint64_t patcheeOffset =
                scanCortexA53Errata843419(ctx, isec, off, limit))
