...
void elf::printHelp(Ctx &ctx) {
  ELFOptTable().printHelp(
      ctx.outs(), (ctx.config->progName + " [options] file...").str().c_str(),
  ...
  ctx.outs() << ctx.config->progName << ": supported targets: elf\n";
}
...
static std::optional<std::string> findFile(Ctx &ctx, StringRef path1,
  ...
  if (path1.starts_with("="))
    path::append(s, ctx.config->sysroot, path1.substr(1), path2);
  else
