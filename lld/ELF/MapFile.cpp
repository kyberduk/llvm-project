...
getSymbolStrings(Ctx &ctx, ArrayRef<Defined *> syms) {
  ...
  parallelFor(0, syms.size(), [&](size_t i) {
    ...
    uint64_t lma = osec ? osec->getLMA() + vma - osec->getVA(ctx, 0) : 0;
...
static void printEhFrame(Ctx &ctx, raw_ostream &os, const EhFrameSection *sec) {
  ...
  for (EhSectionPiece &p : pieces) {
    writeHeader(ctx, os, osec->addr + p.outputOff, osec->getLMA() + p.outputOff,
                p.size, 1);
    os << indent8 << toString(ctx, p.sec->file) << ":(" << p.sec->name << "+0x"
  ...
  for (SectionCommand *cmd : ctx.script->sectionCommands) {
    if (auto *assign = dyn_cast<SymbolAssignment>(cmd)) {
      if (assign->provide && !assign->sym)
        continue;
      uint64_t lma =
          osec ? osec->getLMA() + assign->addr - osec->getVA(ctx, 0) : 0;
    ...
    for (SectionCommand *subCmd : osec->commands) {
      if (auto *isd = dyn_cast<InputSectionDescription>(subCmd)) {
        for (InputSection *isec : isd->sections) {
          ...
          writeHeader(ctx, os, isec->getVA(ctx),
                      osec->getLMA() + isec->outSecOff, isec->getSize(ctx),
                      isec->addralign);
          os << indent8 << toString(ctx, isec) << '\n';
      ...
      if (auto *assign = dyn_cast<SymbolAssignment>(subCmd)) {
        ...
        writeHeader(ctx, os, assign->addr,
                    osec->getLMA() + assign->addr - osec->getVA(ctx, 0),
                    assign->size, 1);
...
static void writeCref(Ctx &ctx, raw_fd_ostream &os) {
  ...
  for (auto kv : map) {
    ...
    print(toString(ctx, *sym), toString(ctx, sym->file));
    for (InputFile *file : files)
      if (file != sym->file)
        print("", toString(ctx, file));
  }
}

void elf::writeMapAndCref(Ctx &ctx) {
  if (ctx.config->mapFile.empty() && !ctx.config->cref)
    return;
  ...
  std::error_code ec;
  StringRef mapFile = ctx.config->mapFile.empty() ? "-" : ctx.config->mapFile;
  raw_fd_ostream os = ctx.openAuxiliaryFile(mapFile, ec);
  if (ec) {
    ctx.error("cannot open " + mapFile + ": " + ec.message());
    return;
  }
